...
class FPDF_AutoWrapTable extends FPDF {
	...
	public function rptDetailData () {
		...
		$this->SetWidths(array(20,60,50,150,90,80,30,60));
    ...
    $this->Cell(60, $h, '', 1, 0, 'C',true);
    $this->Cell(50, $h, '', 1, 0, 'C',true);
    $this->Cell(150, $h, '', 1, 0, 'C',true);
		$this->Cell(90, $h, 'Total', 1, 0, 'L',true);
		$this->Cell(80, $h, '', 1, 0, 'C',true);
    $this->Cell(30, $h, number_format($tsisa,'2','.',''), 1, 0, 'R',true);
    $this->Cell(60, $h, '', 1, 1, 'C',true);
	}
    ...
    function Header()
    {
        ...
        $this->SetFont("", "B", 15);
    		...
    		$this->SetFont("", "B", 12);
        if($_GET['idsub'] == 'TQ'){
    		$this->SetX($left); $this->Cell(0, 10, 'LAPORAN DATA STOK TEST QUALITY', 0, 1,'C');
        }else if ($_GET['idsub'] == 'PACKING'){
        $this->SetX($left); $this->Cell(0, 10, 'LAPORAN DATA STOK PACKING', 0, 1,'C');
        }else if ($_GET['idsub'] == 'ATK'){
        $this->SetX($left); $this->Cell(0, 10, 'LAPORAN DATA STOK ATK', 0, 1,'C');
        }
        ...
        $this->SetX($left += 60); $this->Cell(50, $h, 'Kode', 1, 0, 'C',true);
        $this->SetX($left += 50); $this->Cell(150, $h, 'Nama', 1, 0, 'C',true);
    		$this->SetX($left += 150); $this->Cell(90, $h, 'Jenis', 1, 0, 'C',true);
    		$this->SetX($left += 90); $this->Cell(80, $h, 'Harga', 1, 0, 'C',true);
    		$this->SetX($left += 80); $this->Cell(30, $h, 'Sisa', 1, 0, 'C',true);
        $this->SetX($left += 30); $this->Cell(60, $h, 'Tgl Update', 1, 1, 'C',true);
